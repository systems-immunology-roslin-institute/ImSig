...
pdata=read.csv("blood.csv", header=T)
...
#Nodes Calculator
all <- y[rownames(y) %in% as.character(pdata[pdata$type == "blood", 1]),]
dim(all)
...
for (i in 1: 50)
{
score= ((final1[,2]/final1[,1])*(Edge[,i]/final1[,3])*(Node[,i]/final1[,5]))*100
assign(paste("score", i, sep = "_"), score)
}
score = score_1
for (i in 2:50) score <- cbind(score, get(paste("score",i, sep="_")))

colnames(score) <- paste(rep(0, 50), 50:99, sep='.')

library(plyr)
macrophages= 1- ((count(score[1,] < 10)[2,2])/50)
monocytes= 1- ((count(score[2,] < 10)[2,2])/50)
bcells= 1- ((count(score[3,] < 10)[2,2])/50)
neutrophils=1- ((count(score[4,] < 10)[2,2])/50)
nkcells=1- ((count(score[5,] < 10)[2,2])/50)
plasmacells=1- ((count(score[6,] < 10)[2,2])/50)
platelets=1- ((count(score[7,] < 10)[2,2])/50)
tcells=1- ((count(score[8,] < 10)[2,2])/50)
Ribosomes=1- ((count(score[9,] < 10)[2,2])/50)
IFNs=1- ((count(score[10,] < 10)[2,2])/50)
CellCycle=1- ((count(score[11,] < 10)[2,2])/50)
...
write.table(FinalScore,file="ImSig Score-RNASeq-Blood.txt", sep="\t")